...
filename = file_path_str + '/data/controller_data_train_star.csv' 
f_train = open(filename, 'w+', newline="")
writer_train = csv.writer(f_train, delimiter=',')

filename = file_path_str + '/data/controller_data_test_star.csv' 
...
for idx_problem in range(0,num_problems):
    ...
    for i in range(0,num-1):
        ...
        if len(neighbors_heap) > 0:
            neighbor_entry = neighbors_heap[0] # Selects without popping
        ...
        dz_expand = zs_rrt[z_expand_idx,:] - z_expand
        cost_expand = dz_expand.dot(G_inv_expand).dot(dz_expand)
        node_cost = costs_rrt[z_expand_idx] + cost_expand
        parent_idx = z_expand_idx
        # RECONNECT
        # best near neighbor within ball radius
        neighbors_heap = []
        for nn in range(0,itrs_rrt):
            dz = zs_rrt[nn,:] - z_expand
            dcost = dz.dot(G_inv_expand).dot(dz) # Cost from neighbor to new node
            if dcost < connection_radius:
                heappush(neighbors_heap,(costs_rrt[nn], dcost, nn, dz)) # push 0 in

        for neighbor in neighbors_heap:
            collision = False
            # check collision
            y_CC_expand = sess.run([y_CC], feed_dict={z1_CC: np.reshape(z_expand,(1,2)), z2_CC: np.reshape(zs_rrt[neighbor[2]],(1,2)), 
                                                    dense_CC_conv: conv_CC_out_tiled[0:1]})[0]
            value_CC_expand = 1 / (1 + np.exp(-y_CC_expand))
            isNotFree_expand_t = value_CC_expand < cc_cutoff
            if np.any(isNotFree_expand_t):
                collision = True
            
            # REWIRE IF zp_expand IS A BETTER PARENT
            if not collision and node_cost + neighbor[1] < neighbor[0]:
                new_expand = z_expand
                parents_rrt[neighbor[2]] = itrs_rrt
                costs_rrt[neighbor[2]] = node_cost + neighbor[1]
                T_expand = T_rrt[neighbor[2]]
                traj_exp = np.zeros((T+1, z_dim))
                traj_exp[0,:] = new_expand
                dz_expand = neighbor[3]
                dz_step = dz_expand/T_expand
                for t in range(0,T_expand):
                    new_expand = new_expand + dz_step
                    traj_exp[t+1,:] = new_expand

                trajs_rrt[neighbor[2]] = traj_exp


        # add to tree
        # costs_rrt[itrs_rrt] = costs_rrt[z_expand_idx] + np.linalg.norm(uc_expand)*T_expand # T_expand + 
        costs_rrt[itrs_rrt] = node_cost
        parents_rrt[itrs_rrt] = parent_idx
